from package.gui_aluno import AlunoGUI
from package.persistence import PersistenceManager
from package.user import Aluno
...
class LoginWindow(tk.Tk):
    ...
    def open_signup(self):
        SignupWindow(self, 'Aluno', self.persistence)
    ...
    def authenticate(self):
        ...
        for user in users:
            if isinstance(user, Aluno) and user.email == email and user.password == password:
                self.destroy()
                AlunoGUI(user, self.persistence).mainloop()
                return
